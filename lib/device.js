...
class Device {
  ...
  /**
   * Drain/flush the serial port to clear buffer
   */
  async drainPort() {
    return new Promise((resolve, reject) => {
      if (!this.port) {
  ...
  /**
   * Perform handshake and get device info with retry
   */
  async handshake(retryCount = 1) {
    ...
    for (let attempt = 0; attempt <= retryCount; attempt++) {
      try {
        if (attempt > 0) {
          if (this.verbose) console.log(`Handshake retry attempt ${attempt}/${retryCount}`);
          // Wait a bit before retrying
          await new Promise(resolve => setTimeout(resolve, 500));
        }
        
        // Drain port buffer before handshake to clear any stale data
        try {
          await this.drainPort();
        } catch (drainErr) {
          if (this.verbose) console.warn('Port drain failed:', drainErr.message);
          // Continue anyway, drain failure shouldn't stop handshake
        }
        ...
        if (!response || !response.json) {
          ...
          lastError = new Error('No JSON in handshake response');
          continue;
        }
        ...
        lastError = e;
        if (this.verbose) console.error(`Handshake attempt ${attempt + 1} failed:`, e.message);
        
        // If this is the last attempt, we'll exit the loop and throw
    ...
    // All attempts failed
    if (this.verbose) console.error('All handshake attempts failed');
